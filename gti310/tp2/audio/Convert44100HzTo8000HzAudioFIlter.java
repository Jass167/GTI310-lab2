...
public class Convert44100HzTo8000HzAudioFIlter implements AudioFilter 
{
	...
	public void process() 
	{
		...
		/*si le fichier est un WAVE et il est a 44100HZ on va transformer 
		  le file et on va updater le header*/
            ...
            number.
			 */
			int numberOfSamples= (8*traiteurHeader.getChunk2Size()) 
					/ ( traiteurHeader.getIsStereo()
							*traiteurHeader.getBitParSample() );
			...
			if(traiteurHeader.getBitParSample()==8 
				...
				dataConverter(numberOfSamples,8,1);
			...
			if(traiteurHeader.getBitParSample()==8 
				...
				dataConverter(numberOfSamples,8,2);
			...
			//16bits et mono
			if(traiteurHeader.getBitParSample()==16 
					&& traiteurHeader.getIsStereo()==1){

				dataConverter(numberOfSamples,16,1);
			...
			if(traiteurHeader.getBitParSample()==16 
				...
				dataConverter(numberOfSamples,16,2);
	...
	//Convertir samples
	private void dataConverter(int numberOfSamples,int bitParSample, 
			int isStereo)
	{
